...
class LR_GCCF(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim, weight_size, dropout_list, node_dropout, norm_adj):
        ...
        self.weight_size = weight_size
        self.n_layers = len(self.weight_size)
        self.norm_adj = norm_adj.to(device) # self.user_item_matrix

        self.Linear_list = nn.ModuleList()

        self.weight_size = [self.embedding_dim] + self.weight_size
        for i in range(self.n_layers):
            self.Linear_list.append(nn.Linear(self.weight_size[i], self.weight_size[i+1]))
    ...
    def forward(self):
        ego_embeddings = torch.cat((self.user_embedding.weight, self.item_embedding.weight), dim=0)
        gcn_ego_embeddings = [ego_embeddings]
        for i in range(self.n_layers):
            ego_embeddings = torch.sparse.mm(self.norm_adj, ego_embeddings)
            # sum_embeddings =self.Linear_list[i](side_embeddings)
            # norm_embeddings = F.normalize(ego_embeddings, p=2, dim=1)
            gcn_ego_embeddings.append(ego_embeddings)

        gcn_all_embeddings = torch.cat(gcn_ego_embeddings, dim=-1)
        u_g_embeddings, i_g_embeddings = torch.split(gcn_all_embeddings, [self.n_users, self.n_items], dim=0)
